...
parser.add_argument('--batch-size', type=int, default=40, help='batch size (default: 40)')
parser.add_argument('--ppo-epochs', type=int, default=3, help='ppo epoch (default: 3)')
...
def meta_train():
    ...
    if args.task == 'bandit':
        ...
        sampler = BatchSampler(task, batch_size=args.max_num_traj, num_workers=1)
        num_actions = int(np.prod(sampler.envs.action_space.shape))
        num_states = int(np.prod(sampler.envs.observation_space.shape))
        non_linearity = 'relu'
    else:
        print('Invalid Task')
        return
    ...
    if args.algo == 'reinforce':
        ...
        _, _, _, model = ppo(model, task, num_actions, args.num_tasks, args.max_num_traj, args.max_traj_len,
